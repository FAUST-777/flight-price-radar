...
## 2026-08-05 月份價格帶模型(歷史行情分析)

使用者想要「疫情後歷年每月價格帶+每月最低價」。真實逐月成交價無免費來源(付費航空數據商才有),
改用可系統性取得的替代:Google 每組日期附帶的「歷史常態價格區間」本質就是近年訂票統計,
掃未來 11 個月 × 每月 2 樣本(8 日/22 日)× 6 大日本航點,萃取出每月常態帶。

- `scripts/monthly-bands.mjs`(每週一 07:30 自動跑,刻意與每日掃描器分離自足)
- Sheet `monthly_bands` 分頁**逐週累積**(同日重跑冪等)→ 長期即成自建歷年資料庫
- `api/bands.js` + 網站「📊 日本月份價格帶」區:每月常態區間浮動帶+目前最低圓點,
  六城市+全部(最低)切換,表格檢視含偏高/一般/偏低判斷
- 首掃結論:淡季=9月/11月/12月/1月(常態低 4,900~5,600),旺季=農曆年(常態高 2 萬)、
  10月楓葉、4月櫻花;遠期預售價普遍在常態帶中上緣,促銷艙等接近出發前才釋出
